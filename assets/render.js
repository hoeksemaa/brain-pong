...
window.dash_clientside.renderPong = function(canvasId, gameState, appStatus, n_intervals, interval_ms, freqLeft, freqRight, settings, noBoard) {
    ...
    const PADDLE_WIDTH = (settings && settings.paddle_width) ? settings.paddle_width : 150;
    ...
    // In no-board dev mode, never flicker — no BCI means no SSVEP stimulus needed.
    if (!noBoard) {
    ...
    // AI paddle (dark green — isolated from SSVEP flicker colors)
    ctx.fillStyle = '#006400';
    ctx.fillRect(ai_x - PADDLE_WIDTH / 2, 0, PADDLE_WIDTH, PADDLE_HEIGHT);

    // Player paddle (dark green — isolated from SSVEP flicker colors)
    ctx.fillStyle = '#006400';
